...
READ THIS BEFORE CITING ANY NUMBER ABOVE.
  ...
  * The figures in this README come from the SYNTHETIC stand-in dataset, not
    the real Kaggle file. The generator reproduces the real dataset's
    statistical structure closely, but it is not the real data. Download
    train.csv, rerun, and replace every number with what you actually get.

  * Numbers vary between runs. The notebook version produced a baseline of
    0.600 and a spread of 0.805 +/- 0.027, and the ablation in section 4.5
    flipped sign. That variation is real and expected at n=891 - it is
    exactly what section 5 is about - but it means you should pick one
    canonical run and cite it consistently.

  * The dataset is small. 891 rows, with a 179-row holdout. Every conclusion
    here carries wider error bars than it would at n=100,000. Differences
    under about 2 points are not individually significant.

  * 00_generate_data.py is not real ML work. It exists only because network
    access was unavailable during development. Once you supply the real
    train.csv it does nothing but detect the file and step aside. Do not
    include it if you publish this.

  * No fairness audit. A production model making decisions about people
    would need per-group performance analysis and proxy-variable checks.
    That is out of scope here and deliberately not claimed.
...
The project is only worth what you do next with it. Roughly in order of
difficulty:

  1. Swap in the real Kaggle train.csv and compare every number above.

  2. Add a feature - ticket prefix, cabin deck letter, group size derived
     from shared ticket numbers - and measure whether it earns its place.
     Remember section 4.5: check the delta against the fold std before
     believing it.

  3. Change the metric from accuracy to F1 or AUC in 04_tune.py and see
     whether a different model wins.

  4. Install XGBoost or LightGBM and add it to the SEARCHES dictionary in
     04_tune.py.

  5. Apply this identical five-script structure to a completely different
     dataset.

That last one is the real test. The structure transfers; the dataset does not.
